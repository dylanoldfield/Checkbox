...
function TaskHeader(props:Props) {
    ...
    const sortClicked = (e:React.MouseEvent<HTMLElement>) => {
        e.preventDefault()
        if(props.sortedCol.column !== props.title){
            props.handleClick({column:props.title, dir:SortDirection.Asc})
        }else{
            ...
            props.handleClick({column:props.title, dir:sortD})
        }
    }
    ...
    return (
        ...
        {props.sortHeader? 
            <button onClick={sortClicked}>
                {props.sortedCol.column !== props.title ? 
